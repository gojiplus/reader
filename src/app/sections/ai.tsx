...
// Type guard for errors with digest property
...
export const AiCard = ({
  ...
  const [, _setCurrentSpeakingText] = useState<string | null>(null); // Track the text being spoken/paused
  ...
  const handlePlayPause = () => {
    if (!selectedBook?.textContent || selectedBook.textContent.startsWith('Error loading text:')) {
      toast({
        ...
        description: 'Text content not loaded or is unavailable for playback.',
      });
      return;
    }
    ...
    getCurrentUtteranceText(); // Get text from active utterance

    if (currentlySpeaking) {
      console.warn('[TTS] Requesting pause.');
      pauseSpeech();
      // State updates handled by onPause callback
    } else {
      // If paused
      if (currentlyPaused) {
        console.warn('[TTS] Requesting resume.');
        resumeSpeech();
        // State updates handled by onResume callback
      } else {
        // Otherwise, start speaking the *current* selected book's text from the beginning
        console.warn('[TTS] Requesting play for book:', selectedBook.name);
        _setCurrentSpeakingText(currentBookText); // Track the text we are INTENDING to speak
        speakText(
          currentBookText, // Use the currently selected book's text
          () => {
            // onEnd
            console.warn('[TTS Callback] Playback finished naturally (onEnd).', isSpeakingState);
            setIsSpeakingState(false);
            setIsPausedState(false);
            _setCurrentSpeakingText(null); // Clear tracked text only on natural end from this flow
          },
          errorEvent => {
            // onError
            console.warn('[TTS Callback] Speech error event received.', errorEvent); // Log event for debugging
            // Error type might not always be populated, check message too
            const errorMsg =
              errorEvent.error ||
              (errorEvent instanceof Error ? errorEvent.message : 'Unknown TTS Error');
            console.warn(`[TTS Callback] Error details: ${errorMsg}`);
            ...
            if (errorMsg !== 'interrupted' && errorMsg !== 'canceled') {
              ...
              console.warn(`[TTS Callback] Ignoring expected error: '${errorMsg}'.`);
            }
            console.warn('isSpeakingState', isSpeakingState);
            // Reset state regardless of error type, consistent with tts service logic
            setIsSpeakingState(false);
            setIsPausedState(false);
            _setCurrentSpeakingText(null); // Clear tracked text on any error/stop
          },
          () => {
            // onStart
            console.warn('[TTS Callback] Playback started (onStart).', isSpeakingState);
            setIsSpeakingState(true);
            setIsPausedState(false);
          },
          () => {
            // onPause
            console.warn('[TTS Callback] Playback paused (onPause).', isSpeakingState);
            setIsSpeakingState(false);
            setIsPausedState(true);
          },
          () => {
            // onResume
            console.warn('[TTS Callback] Playback resumed (onResume).', isSpeakingState);
            setIsSpeakingState(true);
            setIsPausedState(false);
          },
          (index: number, boundaries: { start: number; end: number }) => {
            console.warn('Sentence boundary reached:', index, boundaries);
          }
        );
      }
    }
  ...
  const handleSummarize = async () => {
    if (
      ...
      toast({
        variant: 'default',
        title: 'No Text Available',
        description: 'Load or finish loading valid text content before generating summary.',
      });
      return;
    }
    if (!user) {
  ...
  const handleGenerateQuiz = async () => {
    if (
      ...
      toast({
        variant: 'default',
        title: 'No Text Available',
        description: 'Load or finish loading valid text content before generating quiz.',
      });
      return;
    }
    if (!user) {
    ...
    try {
      ...
      console.warn('[Quiz] Requesting quiz generation with input length:', input.text.length);
      const result = await generateQuizQuestions(input);
      console.warn('[Quiz] Quiz generation result:', result);
      ...
      } else if (hasDigest(error)) {
        // Check for Server Component specific errors
  ...
  const handleQuizSubmit = () => {
    ...
    toast({
      ...
      description: `You scored ${score.toFixed(0)}% (${correctCount}/${quizState.data.questions.length}).`,
    });
  };
      ...
      <CardContent className='flex-1 p-4 overflow-auto'>
        <Accordion type='single' collapsible className='w-full' defaultValue='audio'>
            ...
            <AccordionContent>
              <div className='flex items-center justify-center gap-4 py-4'>
                <Button
                  ...
                  aria-label={isSpeakingState ? 'Pause' : 'Play'}
                >
                ...
                <Button
                  ...
                  size='icon'
                  variant='outline'
                  disabled={!isSpeakingState && !isPausedState}
                  aria-label='Stop'
                >
              ...
              {(!selectedBook?.textContent ||
                ...
                !textExtractionState.loading && (
                  <p className='text-sm text-muted-foreground text-center'>
                    Load valid text content first.
                  </p>
                )}
              {textExtractionState.loading && (
                <p className='text-sm text-muted-foreground text-center'>Loading text...</p>
              )}
          ...
          <AccordionItem value='quiz'>
            ...
            <AccordionContent>
              {quizState.error && (
                <p className='text-sm text-destructive break-words'>{quizState.error}</p>
              )}
                ...
                <div className='space-y-6'>
                  {quizSubmitted && quizScore !== null && (
                    <div className='p-3 bg-muted rounded-md text-center'>
                      ...
                      <p className='text-sm text-muted-foreground'>
                        ({((quizScore / 100) * quizState.data.questions.length).toFixed(0)}/
                        {quizState.data.questions.length} correct)
                      </p>
                    </div>
                  )}
                  {quizState.data.questions.map((q, index) => (
                    <div
                      key={`question-${q.question.slice(0, 50).replace(/\s+/g, '-')}`}
                    ...
                    >
                      ...
                      >
                        {q.options.map((opt, i) => {
                          ...
                          return (
                            <div
                              key={`option-${opt.slice(0, 30).replace(/\s+/g, '-')}`}
                              className={cn(
                  ...
                  {!quizSubmitted && (
                    <Button
                      ...
                      disabled={
                        quizState.loading ||
                        Object.keys(userAnswers).length !== quizState.data.questions.length
                      }
                    >
                      Submit Quiz
                    </Button>
              ...
              {!quizState.data &&
                !quizState.error &&
                !quizState.loading && ( // Show generate button only if no data, no error, and not loading
                  <Button
                    onClick={handleGenerateQuiz}
                    size='sm'
                    className='w-full'
