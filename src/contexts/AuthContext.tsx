...
import { Loader2, Terminal } from 'lucide-react'; // For loading state and alert icon
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    console.warn('AuthProvider: Component mounted, setting isClient=true');
    setIsClient(true);
  }, []);
  ...
  useEffect(() => {
    ...
    if (!isClient) {
      console.warn('AuthProvider [Auth Effect]: Skipping effect, not client yet.');
      return;
    }

    console.warn(
      'AuthProvider [Auth Effect]: Running initialization check and auth listener setup.'
    );
    ...
    if (!firebaseConfigValid) {
      const errorMsg =
        initError ||
        'Firebase configuration is invalid (check .env.local or Firebase Studio environment). Auth features disabled.';
    ...
    if (!auth) {
      const errorMsg =
        initError || 'Authentication service failed to initialize unexpectedly after config check.';
      console.error(`AuthProvider [Auth Effect]: ${errorMsg}`);
      setAuthError(errorMsg);
      setLoading(false); // Initialization failed, stop loading
      setUser(null); // Ensure user is null
    ...
    console.warn('AuthProvider [Auth Effect]: Setting up onAuthStateChanged listener.');
    ...
    const unsubscribe = onAuthStateChanged(
      ...
      currentUser => {
        console.warn(
          'AuthProvider [Auth Effect]: Auth state changed.',
    ...
    return () => {
      console.warn('AuthProvider [Auth Effect]: Cleaning up onAuthStateChanged listener.');
      unsubscribe();
    };
  ...
  useEffect(() => {
    ...
    if (isClient && !loading && !authError) {
      console.warn(
        `AuthProvider [Redirect Effect]: Checking redirection. Loading: ${loading}, AuthError: ${authError}, User: ${Boolean(user)}, Path: ${pathname}`
      );
      const isAuthPage = pathname === '/auth';

      if (user && isAuthPage) {
        // Logged in, but on auth page -> redirect to home
        console.warn('AuthProvider [Redirect Effect]: User logged in, redirecting from /auth to /');
        router.push('/');
      } else if (!user && !isAuthPage) {
        // Not logged in, and not on auth page -> redirect to auth
        console.warn('AuthProvider [Redirect Effect]: User not logged in, redirecting to /auth');
        router.push('/auth');
      } else {
        console.warn('AuthProvider [Redirect Effect]: No redirection needed.');
      }
    } else {
      console.warn(
        `AuthProvider [Redirect Effect]: Skipping redirection check. isClient: ${isClient}, loading: ${loading}, authError: ${authError}`
      );
    }
  ...
  if (!isClient) {
    console.warn('AuthProvider: Rendering null (SSR/pre-hydration).');
  ...
  if (loading) {
    console.warn('AuthProvider: Rendering loading indicator.');
    return (
  ...
  if (authError) {
    console.warn('AuthProvider: Rendering auth error message.');
    return (
      <div className='flex items-center justify-center min-h-screen p-4'>
        <Alert variant='destructive' className='max-w-lg'>
          ...
          <AlertDescription>
            Could not initialize or monitor the authentication service. Please ensure your Firebase
            configuration in <code>.env.local</code> (or Firebase Studio environment) is correct and
  ...
  console.warn('AuthProvider: Rendering children.');
  return (
    <AuthContext.Provider value={{ user, loading, authError }}>{children}</AuthContext.Provider>
  );
};
