...
const ExplainInputSchema = z.object({
  text: z
    ...
    .describe('The text to explain.'),
});
export type ExplainInputSchemaInput = z.infer<
  typeof ExplainInputSchema
>;
...
export type ExplainOutputSchemaOutput = z.infer<
...
export async function generateExplanation(input: ExplainInputSchemaInput): Promise<ExplainOutputSchemaOutput> {
  try {
    ...
    if (!outputValidation.success) {
      ...
      const errorDetails = outputValidation.error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join('; ');
