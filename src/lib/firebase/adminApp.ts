...
export function initializeAdminApp(): admin.app.App {
  ...
  if (isInitialized && app) {
    // console.warn('[Firebase Admin] Already initialized successfully.');
    return app;
  }
  // If initialization previously failed, re-throw the stored error immediately
  if (isInitialized && initError) {
    console.error(
      `[Firebase Admin] Attempting to re-initialize after previous failure: ${initError}`
    );
  ...
  console.warn('[Firebase Admin] Attempting Firebase Admin SDK initialization...');
  try {
    ...
    if (!projectId) {
      initError =
        'CRITICAL: Firebase Project ID (NEXT_PUBLIC_FIREBASE_PROJECT_ID) is not set in the server environment for Admin SDK.';
      console.error(`[Firebase Admin] Initialization failed: ${initError}`);
      throw new Error(initError);
    }
    if (!storageBucket) {
      initError =
        'CRITICAL: Firebase Storage Bucket (NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET) is not set in the server environment for Admin SDK.';
      console.error(`[Firebase Admin] Initialization failed: ${initError}`);
      throw new Error(initError);
    }
    console.warn(
      `[Firebase Admin] Using Project ID: ${projectId}, Storage Bucket: ${storageBucket}`
    );
    ...
    try {
      ...
      console.warn(
        '[Firebase Admin] Attempting to load credentials using admin.credential.applicationDefault().'
      );
      credential = admin.credential.applicationDefault();
      credentialSource = 'Application Default Credentials (ADC)';
      console.warn(`[Firebase Admin] Successfully loaded credentials using ${credentialSource}.`);
    } catch (adcError: unknown) {
      const adcErrorMessage = adcError instanceof Error ? adcError.message : 'Unknown ADC error';
      console.warn(
        `[Firebase Admin] Failed to load ADC: ${adcErrorMessage}. Check if ADC are configured or if GOOGLE_APPLICATION_CREDENTIALS is set.`
      );
      // Priority 2: Service Account Path (if GOOGLE_APPLICATION_CREDENTIALS is set)
      if (serviceAccountPath) {
        console.warn(
          `[Firebase Admin] ADC failed. Attempting to load credentials from GOOGLE_APPLICATION_CREDENTIALS path: ${serviceAccountPath}`
        );
        try {
          // Ensure the path exists (basic check)
          // eslint-disable-next-line no-undef
          const fs = require('fs'); // Use require for Node.js environment (this runs only server-side)
          ...
          console.warn(
            `[Firebase Admin] Successfully loaded credentials from ${credentialSource}.`
          );
        } catch (certError: unknown) {
          const certErrorMessage =
            certError instanceof Error ? certError.message : 'Unknown cert error';
          initError = `Failed to load credentials using ADC AND from path ${serviceAccountPath}. ADC Error: ${adcErrorMessage}. Cert Error: ${certErrorMessage}`;
          console.error(`[Firebase Admin] Initialization failed: ${initError}`);
          throw new Error(initError); // Critical failure - couldn't load credentials either way
        }
      } else {
        // ADC failed and no path was provided
        initError = `Failed to load credentials using ADC, and GOOGLE_APPLICATION_CREDENTIALS path was not set. Ensure the server environment has ADC configured (common in Firebase/GCP) or provide a service account key file path via the environment variable. ADC Error: ${adcErrorMessage}`;
        console.error(`[Firebase Admin] Initialization failed: ${initError}`);
    ...
    if (admin.apps.length === 0) {
      console.warn('[Firebase Admin] Calling admin.initializeApp...');
      admin.initializeApp({
        credential,
        projectId, // Explicitly set project ID
        storageBucket, // Set storage bucket
      });
      app = admin.app(); // Get the newly initialized app
      console.warn(
        `[Firebase Admin] SDK initialized successfully for project ${projectId}. App Name: ${app.name}. Credentials loaded via: ${credentialSource}`
      );
    } else {
      console.warn(
        '[Firebase Admin] admin.apps array was not empty before initializeApp. Getting existing default app.'
      );
      ...
      if (app.options.projectId !== projectId || app.options.storageBucket !== storageBucket) {
        console.error(
          `[Firebase Admin] Mismatch! Existing app projectId (${app.options.projectId}) or storageBucket (${app.options.storageBucket}) does not match environment config (${projectId}, ${storageBucket}). This might cause issues.`
        );
        // Decide how to handle mismatch: throw error, log warning, etc.
        // For now, we'll log and proceed with the existing app.
      } else {
        console.warn(
          `[Firebase Admin] Existing app configuration matches environment. Using existing app: ${app.name}`
        );
      }
    }
  } catch (error: unknown) {
    ...
    console.error(
      '[Firebase Admin] CRITICAL INITIALIZATION FAILURE:',
      initError,
  ...
  if (!app) {
    initError =
      initError || 'Firebase Admin app instance is unexpectedly null after initialization attempt.';
