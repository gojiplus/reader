...
if (typeof window !== 'undefined') {
  ...
  console.warn(`[PDF.js] Attempting to set worker source to: ${workerSrc}`);
  try {
    ...
    if (pdfjsLib.GlobalWorkerOptions.workerSrc !== workerSrc || !workerSrcIsSet) {
      ...
      console.warn('[PDF.js] Worker source updated successfully.');
      workerSrcIsSet = true; // Mark as set
      workerSetupError = null; // Clear any previous setup error
    } else {
      console.warn('[PDF.js] Worker source already set to the correct path.');
      workerSrcIsSet = true; // Ensure it's marked as set
    }
    // Verify if the setting took effect (optional sanity check)
    console.warn(`[PDF.js] Current workerSrc: ${pdfjsLib.GlobalWorkerOptions.workerSrc}`);
  } catch (e) {
    workerSetupError = new Error(
      `Error setting PDF.js worker source: ${e instanceof Error ? e.message : String(e)}`
    );
...
export async function convertFileToText(file: File, user: User): Promise<string> {
  ...
  if (typeof window !== 'undefined' && (!workerSrcIsSet || workerSetupError)) {
    ...
    throw new Error(
      `PDF processing cannot start. Worker setup failed: ${errorMsg}. Check console for details.`
    );
  }
  ...
  console.warn(`[PDF.js] Starting text extraction from PDF: ${file.name}`);
  ...
  try {
    ...
    console.warn(
      `[PDF.js] Using workerSrc for getDocument: ${pdfjsLib.GlobalWorkerOptions.workerSrc}`
    );

    const loadingTask = pdfjsLib.getDocument({ data: arrayBuffer });
    pdf = await loadingTask.promise; // Assign to pdf variable
    console.warn(`[PDF.js] PDF loaded: ${pdf.numPages} pages`);
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      try {
        ...
        console.warn('fixedText', fixedText);
        fullText += `${fixedText}\n\n`; // Add double newline between pages
        console.warn(`[PDF.js] Extracted text from page ${i}`);
      } catch (pageError) {
    ...
    console.warn(`[PDF.js] Finished extracting text. Total length: ${fullText.length}`);
    ...
    if (error instanceof Error) {
      ...
      if (error.name === 'PasswordException' || specificMessage.includes('password')) {
        ...
        specificMessage.includes('Invalid PDF structure')
      ) {
        specificMessage = `Could not extract text. The file "${file.name}" is not a valid PDF or is corrupted.`;
      } else if (
        specificMessage.includes('Setting up fake worker failed') ||
        specificMessage.includes('Failed to fetch dynamically imported module')
      ) {
        const workerPath = pdfjsLib.GlobalWorkerOptions.workerSrc || 'Not Set';
        console.error(
          '[PDF.js] Worker setup or loading failed critically. Check the workerSrc path in logs, network tab for 404s, and Webpack config.',
          workerPath,
          error
        );
    ...
    if (pdf) {
      try {
        await pdf.destroy();
        console.warn('[PDF.js] PDF document destroyed.');
      } catch (destroyError) {
